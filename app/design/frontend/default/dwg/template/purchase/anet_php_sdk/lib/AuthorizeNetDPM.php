...
class AuthorizeNetDPM extends AuthorizeNetSIM_Form
{
    ...
    public static function directPostDemo($url, $api_login_id, $transaction_key, $amount = "0.00", $md5_setting = "")
    {
        ...
        elseif (count($_POST)) 
        {
            ...
            {
                echo "Error -- no7t AuthorizeNet. Check your MD5 Setting.";
            }
        }
     ...
     * @return string
     */
    public static function getCreditCardForm($amount, $fp_sequence, $relay_response_url, $api_login_id, $transaction_key, $test_mode = false, $prefill = true, $prefillArray,$regionCode) //test_mode= true
    {
        $time = time();
        ...
        <form method="post" action="'.$post_url.'">
            ...
            <fieldset>
                <div>
                    ...
                    <input type="text" class="text" size="18" name="x_card_num" value="'.($prefill ? '' : '').'"></input>
                </div>
                <div id="expDate">    
                    <label>Exp. (mm/yy)</label>
		    <select id="setMonth" class="textFormat"></select><select id="setYear" class="textFormat"></select>
                    <input type="text" class="text" size="4" name="x_exp_date" id="setDate" value="'.($prefill ? '' : '').'"></input>
                </div>
                <div>
                    <label>CCV <a id="ccvhelp" style="cursor:pointer;"> ?</a></label>
            ...
            <fieldset>
                <div>
                    ...
                    <input type="text" class="text" size="15" name="x_first_name" value="'.($prefill ? $prefillArray->getFirstname() : '').'"></input>
                </div>
                <div>
                    <label>Last Name</label>
                    <input type="text" class="text" size="14" name="x_last_name" value="'.($prefill ? $prefillArray->getLastname() : '').'"></input>
                </div>
            </fieldset>
            <fieldset>
                <div>
                    <label>Address</label>
                    <input type="text" class="text" size="26" name="x_address" value="'.($prefill ? $prefillArray['street'] : '').'"></input>
                </div>
                <div>
                    <label>City</label>
                    <input type="text" class="text" size="15" name="x_city" value="'.($prefill ? $prefillArray->getCity() : '').'"></input>
                </div>
            </fieldset>
            <fieldset>
                <div>
                    <label>State</label>
                    <input type="text" class="text" size="4" name="x_state" value="'.($prefill ? $regionCode : '').'"></input>
                </div>
                <div>
                    <label>Zip Code</label>
                    <input type="text" class="text" size="9" name="x_zip" value="'.($prefill ? $prefillArray->getPostcode() : '').'"></input>
                </div>
                <div>
                    <label>Country</label>
                    <input type="text" class="text" size="22" name="x_country" value="'.($prefill ? $prefillArray->getCountryId() : '').'"></input>
                </div>
            </fieldset>
            <input type="submit" value="Complete Purchase" class="btn-cart btn-cart-standalone btn-cart-hover">
        </form>'; //took out class submit buy from submit button
        return $form;
    }
